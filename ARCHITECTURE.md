...
This document is the map of the system. It is written to be equally useful to a human reviewer and to an AI agent asked to extend the codebase. **If a contract is not described here or exported from `@teradata-pe/bridge`, it does not exist.**
...
- The shell consumes remotes **only** via their MF 2.0 manifests, resolved at boot from the environment registry (`src/app/remote-registry.ts`) and registered with `registerRemotes()` — never declared at build time.
- Remotes consume the host **only** via `getBridge()` from `@teradata-pe/bridge`.
...
| `@teradata-pe/bridge` | workspace (`requiredVersion: false`) | The contract itself. Identity is *additionally* anchored to `globalThis[Symbol.for('up-ui.bridge.v1')]`, so even a duplicated module copy resolves the same bridge. |
...
| --- | --- | --- |
| `EventBus` (`BridgeEventMap`) | host instance | `on/once/emit/onAny`. All cross-remote events are declared in `BridgeEventMap` — add events there first. |
| `AuthSession` | shell (`AuthSessionController` stays host-private) | read-only `user`, `can(permission)`, `authorizeRequest(init)`, `subscribe`. **Tokens live in a host closure; remotes never store them.** |
...
| `SharedDataCache` (SWR) | host instance | `fetch(key, fetcher, {staleMs, ttlMs})` with in-flight dedupe → cross-remote requests for the same key cost one network call. `peek/set/invalidate/subscribe/keys`. |
| `AsyncOperationManager` | host instance | `start(descriptor)` — the poll closure executes in the host, survives remote unmount, broadcasts `operation:*` events, lands results in the `SharedDataCache` under `cacheKey`, and carries a `resultRoute` for the host's toast deep-link. |
...
**Boot order:** the shell calls `initHostBridge()` (→ `createBridge()` + `installBridge()`) *before* Angular bootstraps and before any remote loads. Standalone remotes install a local dev bridge in their `bootstrap` file when `!hasBridge()` — same contracts, no host.
...
4. **zone.js is currently loaded** — the Angular-Rsbuild toolchain appends it to the browser polyfills unconditionally. Whether that *forces* zone-based change detection is a separate question and has not been tested here: `provideZonelessChangeDetection()` instructs Angular to ignore `NgZone` regardless of whether the polyfill is present. The extracted shell tests this explicitly (ADR-015).
...
1. New cross-remote event? Add it to `BridgeEventMap` in `packages/bridge/src/schema/types.ts` first.
...
3. New remote? Copy a remote's `rsbuild.config.ts` (keep the shared map identical), expose a `./mount` (non-Angular) or `./routes` (Angular), add an entry to `registry/remotes.dev.json` (the shell picks it up at boot — no shell rebuild), add its route to `app.routes.ts`, and declare its types in `remotes.d.ts`.
4. New protected route? Compose validators from `@teradata-pe/bridge`; never hand-roll permission checks in a remote.
